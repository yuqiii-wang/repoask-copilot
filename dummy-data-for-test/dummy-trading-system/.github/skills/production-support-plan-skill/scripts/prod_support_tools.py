...
-----------
  ...
  discover-loki  [--loki-url <url>]
      Discover available log streams from Loki.
      Output: JSON array with stream selectors and labels
...
def _discover_loki(args: argparse.Namespace) -> None:
    """Discover available log streams from Loki."""
    loki_client = _load("loki_client", "loki_client.py")
    try:
        streams = loki_client.discover_log_streams(args.loki_url)
        print(json.dumps(streams, indent=2, ensure_ascii=False))
    except Exception as exc:
        print(f"ERROR: could not reach Loki server — {exc}", file=sys.stderr)
        sys.exit(2)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # discover-loki
    p_loki = sub.add_parser("discover-loki", help="Discover log streams from Loki")
    p_loki.add_argument("--loki-url", default="http://localhost:8094",
