...
async def scan_logtail_unified(logtail_queries) -> dict[str, dict[str, list[str]]]:
    """
    Execute Logtail queries from the unified query builder format.
    
    Args:
        logtail_queries: List of LogtailQuery NamedTuples from query_builders module
        
    Returns:
        Dict mapping component to {keyword: [timestamps]}
    """
    results: dict[str, dict[str, list[str]]] = {}
    
    async def fetch_query(query):
        """Fetch a single Logtail query and extract timestamps."""
        try:
            async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
                ...
                qs = parse_qs(parsed.query)
                keyword = qs.get('i', ['all'])[0]
                
                return query.component, keyword, timestamps
        except Exception as e:
            print(f"Error fetching {query.url}: {e}", file=sys.stderr)
            return query.component, "error", []
    
    # Execute all queries in parallel
    query_results = await asyncio.gather(*[fetch_query(q) for q in logtail_queries])
    
    # Group by component
    for component, keyword, timestamps in query_results:
        if component not in results:
            results[component] = {}
        
        if keyword not in results[component]:
            results[component][keyword] = []
        
        results[component][keyword].extend(timestamps)
    
    # Sort and deduplicate
    for component in results:
        for keyword in results[component]:
            results[component][keyword] = sorted(set(results[component][keyword]))
