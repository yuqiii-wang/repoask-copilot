...
public class MarketDataServiceImpl implements MarketDataService {
    ...
    private final MarketDataFeignClient marketDataFeignClient;
    private final FaultInjectionService faultInjectionService;

    public MarketDataServiceImpl(MarketDataFeignClient marketDataFeignClient,
                                 FaultInjectionService faultInjectionService) {
        this.marketDataFeignClient = marketDataFeignClient;
    ...
    @Recover
    public MarketData recoverGetMarketData(DownstreamTimeoutException ex, String symbol) {
        logger.error("[RETRY-EXHAUSTED] getMarketData timed out for symbol={}: {}", symbol, ex.getMessage());
        return generateMockMarketData(symbol);
    }

    @Recover
    public MarketData recoverGetMarketData(MissingArgumentException ex, String symbol) {
        logger.error("[RETRY-EXHAUSTED] getMarketData missing arg for symbol={}: {}", symbol, ex.getMessage());
        return generateMockMarketData(symbol);
    }

    @Recover
    public MarketData recoverGetMarketData(DownstreamServerException ex, String symbol) {
        logger.error("[RETRY-EXHAUSTED] getMarketData downstream 5xx for symbol={}: {}", symbol, ex.getMessage());
        return generateMockMarketData(symbol);
    }

    @Recover
    public MarketData[] recoverGetMarketDataBatch(DownstreamTimeoutException ex, String[] symbols) {
        logger.error("[RETRY-EXHAUSTED] getMarketDataBatch timed out (symbols={}): {}", symbols.length, ex.getMessage());
        return buildFallbackBatch(symbols);
    }

    @Recover
    public MarketData[] recoverGetMarketDataBatch(MissingArgumentException ex, String[] symbols) {
        logger.error("[RETRY-EXHAUSTED] getMarketDataBatch missing arg: {}", ex.getMessage());
        return buildFallbackBatch(symbols);
    }

    @Recover
    public MarketData[] recoverGetMarketDataBatch(DownstreamServerException ex, String[] symbols) {
        logger.error("[RETRY-EXHAUSTED] getMarketDataBatch downstream 5xx: {}", ex.getMessage());
        return buildFallbackBatch(symbols);
    }
    ...
    public MarketData getMarketData(String symbol) {
        ...
        faultInjectionService.checkFault("market-data.quote");

        // ── Guard: required fields ───────────────────────────────────────────
        if (symbol == null || symbol.isBlank()) {
            logger.error("[FAULT] getMarketData called with null/blank symbol");
        ...
        if (data == null || data.getTimestamp().plusSeconds(30).isBefore(LocalDateTime.now())) {
            // Fetch from external market data feed; fall back to mock on any error
            try {
                logger.debug("Fetching live quote from external feed: symbol={}", symbol);
                ExternalMarketQuoteResponse quote = marketDataFeignClient.getQuote(symbol);
                data = mapToMarketData(quote);
                logger.info("External market data received: symbol={}, source={}, status={}",
                        symbol, quote.getExchange(), quote.getStatus());
            } catch (Exception e) {
                logger.warn("External market data feed unavailable for symbol={}, falling back to mock data: {}",
                        symbol, e.getMessage());
    ...
    public MarketData[] getMarketDataBatch(String[] symbols) {
        // ── Chaos: fault injection ───────────────────────────────────────────
        faultInjectionService.checkFault("market-data.batch");

        // ── Guard: required fields ───────────────────────────────────────────
        if (symbols == null || symbols.length == 0) {
            logger.error("[FAULT] getMarketDataBatch called with null/empty symbols array");
            throw new MissingArgumentException("symbols");
        }
    ...
    private MarketData mapToMarketData(ExternalMarketQuoteResponse quote) {
        MarketData data = new MarketData();
        data.setSymbol(quote.getSymbol());
        data.setExchange(quote.getExchange());
        data.setLastPrice(quote.getLastPrice());
        data.setBid(quote.getBid());
        data.setAsk(quote.getAsk());
        data.setOpenPrice(quote.getOpenPrice());
        data.setHighPrice(quote.getHighPrice());
        data.setLowPrice(quote.getLowPrice());
        data.setPreviousClose(quote.getPreviousClose());
        data.setVolume(quote.getVolume());
        data.setTimestamp(quote.getTimestamp() != null ? quote.getTimestamp() : LocalDateTime.now());
        return data;
    }
    ...
    private MarketData[] buildFallbackBatch(String[] symbols) {
        if (symbols == null) return new MarketData[0];
        MarketData[] results = new MarketData[symbols.length];
        for (int i = 0; i < symbols.length; i++) {
            results[i] = generateMockMarketData(symbols[i]);
        }
        return results;
    }
}
