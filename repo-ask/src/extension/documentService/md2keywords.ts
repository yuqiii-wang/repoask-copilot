...
import { SKIP_WORDS, extract_capital_sequences } from './tokenization2keywords/patternMatch';
import { wordsFromText, buildNGrams } from './tokenization2keywords';
 ...
 */
function addNGramsFromText(text: any, keywords: any, maxN: any) {
 ...
 */
function addCodeIdentifiers(text: any, keywords: any) {
 ...
 */
function extractMdKeywords(text: any) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (token.type === 'heading_open') {
            ...
            const maxN = (HEADING_MAX_NGRAM as Record<number, number>)[level] || 2;
            const inline = tokens[i + 1];
            if (inline && inline.type === 'inline' && inline.children) {
                const headingText = inline.children.map((c: any) => c.content || '').join(' ');
    ...
    return [...keywords].filter((kw: any) => (kw as string).length >= 2);
}

export {  extractMdKeywords };
