import { SKIP_WORDS } from './patternMatch';
 ...
 */
function splitCamelCase(word: any) {
    return word
 ...
 * @returns {string[]}
 */
function wordsFromText(text: any) {
 ...
 * @returns {string[]}
 */
function buildNGrams(words: any, minN = 1, maxN = 4) {
 ...
 * @returns {string[]}
 */
function tokenize(text: any, options: { includeNGrams?: boolean; minN?: number; maxN?: number } = {}) {
 ...
 * @returns {string[]}
 */
function extractMermaidKeywords(mermaidText: any) {
 ...
 */
function tokenizationMain(text: any) {
    const words = wordsFromText(String(text || ''));
    return buildNGrams(words, 1, 4);
}

export {  tokenize, generateSynonyms, wordsFromText, buildNGrams,  extractMermaidKeywords, tokenizationMain };
