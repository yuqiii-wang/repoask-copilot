import { extractMdKeywords } from './md2keywords';
import { SKIP_WORDS } from './tokenization2keywords/patternMatch';
...
export default function(context: any) {
     ...
     */
    function normalizeKeywordsInput(keywords: any) {
     ...
     */
    function cleanKeywords(keywords: any, limit: any) {
     ...
     */
    function normalizeCategorizedKeywords(keywords: any) {
        if (Array.isArray(keywords)) {
     ...
     */
    function flattenCategorizedKeywords(keywords: any) {
        if (Array.isArray(keywords)) return cleanKeywords(keywords, undefined);
        ...
        for (const cat of ['title', 'structural', 'semantic', 'bm25', 'kg', 'synonyms']) {
            ...
            const items = Array.isArray(slot)
                ? cleanKeywords(slot, undefined)
                : ['1gram', '2gram', '3gram', '4gram'].flatMap(g => {
                    const gs = slot[g];
                    if (!gs) return [];
                    if (Array.isArray(gs)) return cleanKeywords(gs, undefined);
     ...
     * full diagram source.
     */
    function extractMermaidEntityTokens(mermaidText: any) {
     ...
     */
    function mergeSemanticKeywords(keywords: any, semanticKeywords: any, limit = DEFAULT_KEYWORD_LIMIT) {
     ...
     */
    function appendKeywordsToExisting(existing: any, newKws: any, limit = DEFAULT_KEYWORD_LIMIT) {
        return cleanKeywords([...cleanKeywords(existing, undefined), ...cleanKeywords(newKws, undefined)], limit);
    }
     ...
     */
    function normalizeMetadataKeywordFields(metadata: any) {
     ...
     */
    function buildKeywordOnlyIndexText(metadata: any) {
