...
async function generateSummaryText(vscodeApi, inputText) {
    if (!vscodeApi.lm || !vscodeApi.LanguageModelChatMessage) return inputText;
    try {
        const sharedMod = require('../chat/shared');
        const model = await sharedMod.selectDefaultChatModel(vscodeApi);
        ...
        const text = await shared.collectResponseText(vscodeApi, response);
...
module.exports = function (context) {
     ...
     */
    function resolvePrimaryDoc(docId, confluencePageId, jiraId, confluenceLink) {
        const allMetadata = readAllMetadata(storagePath);
        if (docId) {
            const found = allMetadata.find(d => String(d.id) === String(docId));
            if (found) return found;
        }
        if (confluencePageId) {
            const found = allMetadata.find(d => String(d.id) === String(confluencePageId));
            if (found) return found;
        }
        if (jiraId) {
            const found = allMetadata.find(d =>
        ...
        if (confluenceLink) {
            try {
                ...
                if (m && m[1]) return allMetadata.find(d => String(d.id) === String(m[1])) || null;
            } catch (_) {}
        }
        return null;
    }
     ...
     */
    async function saveSummary(docId, summary) {
