...
import { updateConfluencePage } from '../mcp/confluenceApi';
import { mapFeedbackError } from './errMap';
import { createOpenDocCommand, createMetadataCommands, createSearchCommand, createPromptsCommand, createSkillsCommand, createDeleteCommand, createResetCommand } from './commands';

const _md = new MarkdownIt({ html: false, linkify: true, breaks: true });

function rewriteMarkdownImageLinksForWebview(markdown: string, _docId: string | number, _storagePath: string, _webview: any): string {
    // Image links are left as relative paths; the webview renders them as plain text fallback
    return markdown;
}

function renderMarkdownForWebview(markdown: string): string {
    return _md.render(markdown);
}

function createSidebarController(deps: any) {
    const {
        vscode,
        context,
        storagePath,
    ...
    const sidebarProvider = {
        resolveWebviewView: async (webviewView: any) => {
            try {
                ...
                docsWebviewView.webview.options = {
                    ...
                    localResourceRoots: [
                        ...
                        vscode.Uri.joinPath(context.extensionUri, 'out', 'sidebar'),
                        vscode.Uri.file(storagePath)
                    ]
                };

                docsWebviewView.webview.onDidReceiveMessage(async (message: any) => {
                    ...
                    if (message?.command === 'getDocumentByID' && message.id) {
                        ...
                        const document = allMetadata.find((doc: any) => String(doc.id) === String(message.id));
                        if (document) {
                    ...
                    if (message?.command === 'openDocStore') {
                        ...
                        try {
                            const docStorePath = storagePath;
    ...
    function setSidebarSyncStatus(message: any) {
    ...
    function setSidebarSyncError(message: any) {
    ...
    function upsertSidebarDocument(metadata: any) {
    ...
    function revealDocumentInSidebar(docId: any) {
        ...
        const metadata = readAllMetadata(storagePath).find((doc: any) => String(doc.id) === id);
        if (!metadata) {
            return false;
        }

        const rawContent = readDocumentContent(storagePath, metadata.id) || 'No local markdown content found.';
        const content = rewriteMarkdownImageLinksForWebview(rawContent, metadata.id, storagePath, docsWebviewView.webview);
    ...
    function getSidebarHtml(webview: any) {
        const nonce = require('crypto').randomBytes(16).toString('base64');
        const htmlPath = vscode.Uri.joinPath(context.extensionUri, 'src', 'sidebar', 'index.html');
        const cssPath = vscode.Uri.joinPath(context.extensionUri, 'out', 'sidebar', 'styles.css');
        const appJsPath = vscode.Uri.joinPath(context.extensionUri, 'out', 'sidebar', 'sidebarApp.js');

        const htmlTemplate = fs.readFileSync(htmlPath.fsPath, 'utf8');

        const cssUri = webview.asWebviewUri(cssPath).toString();
        const appJsUri = webview.asWebviewUri(appJsPath).toString();

        const csp = `default-src 'none'; style-src ${webview.cspSource}; script-src 'nonce-${nonce}' ${webview.cspSource}; img-src ${webview.cspSource} data:; font-src ${webview.cspSource};`;

        const docs = readAllMetadata(storagePath).sort((a: any, b: any) => String(b.last_updated).localeCompare(String(a.last_updated)));
    ...
    function getSidebarErrorHtml(message: any) {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    async function handleSubmitFeedback(feedbackPayload: any) {
        try {
            ...
            if (sourceQuery && (confluencePageId || jiraId || confluenceLink)) {
                try {
                    ...
                    if (confluencePageId) {
                        targetDocument = allMetadata.find((doc: any) => String(doc.id) === String(confluencePageId));
                    }
                    if (!targetDocument && jiraId) {
                        targetDocument = allMetadata.find((doc: any) => String(doc.id) === String(jiraId));
                    }
                    if (!targetDocument && confluenceLink) {
                        try {
                            ...
                            if (pageIdMatch && pageIdMatch[1]) {
                                targetDocument = allMetadata.find((doc: any) => String(doc.id) === String(pageIdMatch[1]));
                            }
                        } catch (urlError) {
                    ...
                    if (targetDocument) {
                        ...
                        // referenceQueries collected but not used downstream
                    } else {
            ...
            if (confluencePageId || jiraId || confluenceLink) {
                try {
                    ...
                    if (confluencePageId) { primaryDoc = allMetadata.find((d: any) => String(d.id) === String(confluencePageId)); }
                    if (!primaryDoc && jiraId) { primaryDoc = allMetadata.find((d: any) => String(d.id) === String(jiraId)); }
                    if (!primaryDoc && confluenceLink) {
                        try {
                            ...
                            if (linkPageIdMatch && linkPageIdMatch[1]) {
                                primaryDoc = allMetadata.find((d: any) => String(d.id) === String(linkPageIdMatch[1]));
                            }
                        } catch (_) {}
                    }
                    if (primaryDoc) {
                        ...
                        if (content) {
                            ...
                            const filteredSecondary = Array.isArray(feedbackPayload.secondaryUrls)
                                ? feedbackPayload.secondaryUrls.filter((u: any) => String(u || '').trim() && String(u) !== 'none')
                                : [];
...
async function getVSCodeUsername() {
    ...
    async function showLogActionButton(firstUserQuery: any, firstRankedDocUrl: any, fullAiResponse: any, selectedDocument: any, queryStartTime: any) {
...
export {  createSidebarController };
