module.exports = function createMetadataCommands(deps) {
    ...
    function normalise(msg) {
        return {
            ...
            secondaryUrls: Array.isArray(msg.secondaryUrls)
                ? msg.secondaryUrls.map(u => String(u || '').trim()).filter(Boolean)
                : []
        };
    }
     ...
     */
    async function generateSummary(message, docsWebviewView, upsertSidebarDocument) {
     ...
     */
    async function generateKnowledgeGraph(message, docsWebviewView, upsertSidebarDocument) {
        const { docId, conversationSummary, confluencePageId, jiraId, confluenceLink,
                sourceQuery, existingKnowledgeGraph, secondaryUrls } = normalise(message);
